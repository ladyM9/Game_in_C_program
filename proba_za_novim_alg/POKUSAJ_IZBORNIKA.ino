...
bool collided_with(struct OBJECT o, struct RECT r, struct  DISTANCE d, struct LINE l) {
  ...
  if ((OBJECT1.x >= (l.m) && (OBJECT1.x >= l.m1)) && (OBJECT1.y >= (l.n || l.n1))) _collisionDetected = true;  //detekcija na izlaz iz labirinta
  if (_collisionDetected ) {
    //display.clearDisplay();
    //display.setCursor(0, 0);
    //display.setTextColor(BLACK, WHITE);
...
bool odabir_lab(int x, struct OD t, struct OBJECT o) {
  display.clearDisplay();
  display.setCursor(0, 0);
  display.setTextColor(BLACK, WHITE);
