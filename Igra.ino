...
void setup()
{
    ...
    ball.firstBallposition(display);
}
...
void loop()
{
    ...
    if (GAME(level) == true)
    {
        if ((unsigned long)(millis() - change_Timer) >= TIMER_CHANGE_INTERVAL)
        {
            change_Timer = millis();
            level++;
        }
...
bool GAME(int _p)
{
    ...
    case 0:
        ...
        if ((ball.checkExit(exitLine, labExitEl)) == true)
        {
            g = true;
        }
        return g;
    ...
    case 1:
        ...
        ball.firstBallposition(display);
        int a = maze.LoadNewMaze();
        maze.drawLines(display, labirint[a], labElements);
        GameLoop();
