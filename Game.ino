...
void loop() {
  ...
  if (ref == true) {
    ...
    int col = checkCollision(OBJECT1 , myLabs[0], 55);
    int col_1 = checkCollision(OBJECT1 , myLabs[6], 1);
    int col_2 = checkPoint(OBJECT1 , myLabs[0],55);
    ...
    if ( col == 0)
    {
      drawLines(myLabs[0], 55 , ILI9341_WHITE);

    }
    if ( col != 0)
    {
      drawLines(myLabs[0], 55 , ILI9341_RED);
    }
    if ( col_1 != 0)
    {
      //display.clearDisplay();
