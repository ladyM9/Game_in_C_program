...
int Button::myButton(Adafruit_ILI9341 &lcd, int _xcur, int _ycur, int x, int y, int w, int h, int _textscale, char *mytext, int draw) // metoda pomocu koje ispisujem pojedinu tipku na display
{
    ...
    int push_button = HIGH; // varijabla koja govori da li je moja tipka pritisnuta ili nije, u prvom trenutku nije pritisnuta
    ...
    if ((xp > x) && (xp < (w + x)) && (yp > y) && (yp < (h + y)) && ((push_button) && (!pushb))) // ako je kursor postavljen na tipku i ako je pushb pritisnut
    {

        push = 1; // push_button je false odnosno tipka na displayu je pritisnuta
