...
struct RECT {
  ...
  int rectlocy = 0; //
  int rectlocx1 = rectlocx - rectw;
  int rectlocy1 = rectlocy + recth;
  int distance = 0;
} RECT1;
...
void loop() {
  ...
  if (ref) {
    ...
    delay(100);
    collided_with(OBJECT1, RECT1, DISTANCE1);
    //delay(1000);
    display.display();
  }

}
void collided_with(struct OBJECT o, struct RECT r, struct  DISTANCE d) {
  d.m = abs(o.x - r.rectlocx);
  d.n = abs(o.y - r.rectlocy);
  d.e = abs(o.x + r.rectlocx);
  d.t = abs (o.y + r.rectlocy);
  if ((d.m > ( r.rectw  + o.r)) || (d.n > (r.recth  + o.r)) && (d.m  <=  (r.rectw) ||  (d.n <= (r.recth))) || ( d.e < ( r.rectw + o.r)) || (d.t < (r.recth + o.r))){
